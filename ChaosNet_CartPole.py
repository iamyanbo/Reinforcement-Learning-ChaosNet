...
def normalize(x):
    """
    Normalizes each element in the input array to be between [0,1).
    Here, we normalize each element to be between [0,1) based on the context of the environemnt:
    index 0: min -4.8, max 4.8
    index 1: -inf, inf
    index 2: min -0.418, max 0.418
    index 3: -inf, inf
    """
    x[0] = (x[0] + 4.8) / 9.6
    x[1] = 1 / (1 + np.exp(-x[1]))
...
def test(b, q):
    model = ChaosNet(num_neurons=4, skew_map_type='Sk-B', feature_extraction_method='TT-SS', num_labels=2, epsilon=0.01, b=b, time_series_length=100000, a=0, c=1, q=q)
        ...
        while not done:
            ...
            if not done:
                if reward > 0:
                    ...
                    # label is size of action_space + action for negative reward
...
def tune_hyperparameters():
    ...
    for i in range(num_b_iterations):
        for j in range(num_q_iterations):
            ...
            best_avg_score, scores, _ = test(b, q)
...
if __name__ == '__main__':
    ...
    # 0.11, 0.61
    avg_score, scores, avg_internal_representation = test(0.11, 0.61)
    print(avg_score)
    print(scores)
